...
- Compare evidence by `population_profile`, `tadalafil_exposure`, `study_design`, `comparator_context`, `endpoint_domain`, `effect_estimate`, `safety_findings`, `directness_to_target_population`, and `confounding_and_bias_limits`.
- Synthesize benefits and risks together; do not create a benefit-only summary for off-label use.
- Separate clinical outcomes from surrogate biomarkers, mechanistic plausibility, nonhuman evidence, and adjacent PDE5-inhibitor class evidence.
...
- The report distinguishes direct tadalafil evidence from adjacent drug-class or mechanistic evidence.
