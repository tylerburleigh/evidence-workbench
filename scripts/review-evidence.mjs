...
const draftEvidenceReviewsRoot = path.join(researchRoot, "drafts", "evidence-reviews");
...
function buildDraftReview({
  ...
  if (reviewerKind === "human") {
    draftReview.reviewer_id = reviewerId ?? "local-curator";
  } else {
    draftReview.skill_name = "evidence-review";
    if (reviewerId) {
...
async function validateReviewShape(review, bundle, domainPack) {
  ...
  if (!domainPack.reviewLaneIds.has(review.review_lane)) {
    fail(`Unsupported review lane for active domain pack: ${review.review_lane}`);
  }
...
async function commandApply(options) {
  ...
  const domainPack = await loadActiveDomainPack();
  await validateReviewShape(draftReview, bundle, domainPack);
